...
/* Plan of Attack 
    - List = Array 
    - Create variable = store numbers
    - Loop through my array of inputs
    - Each input, check for number (two pointers)
    - Reduce function on variable of numbers 
    - Return

 --> base cases <--
    - if only one digit, use it twice
    - if no digits = 0; 
    - if more than two digits, only take first & last

---> other <---
    - convert data input into an array of strings
...
const dayOne = (array) => {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    // extract the numbers only & push into an array called NumbersOnly
    let numbersArray = input.match(/\d+/g);
    numbersOnly.push(numbersArray ? numbersArray.join("") : "");
  }

  for (let i = 0; i < numbersOnly.length; i++) {
    let currentInput = numbersOnly[i];
    if (currentInput.length === 2) {
      numbersOnly[i] = parseInt(numbersOnly[i], 10);
      continue;
    }
    if (currentInput.length > 2) {
      let first = currentInput[0];
      let last = currentInput[currentInput.length - 1];
      numbersOnly[i] = parseInt(first + last, 10);
    }

    if (currentInput.length === 1) {
      numbersOnly[i] = parseInt(currentInput + currentInput, 10);
    }
  }

  let value = numbersOnly.reduce(
...
const dataArray = puzzleInput.split("\n").map((line) => line.trim());
